...
:::danger
Vue 3 的生命週期名稱和 Vue 2 不同  
Vue 2 的 `destroyed` 在 Vue 3 是 `unmounted`
:::

比較需要注意的生命週期有：
- `created` Vue 元件被建立，但是還沒有渲染在 DOM，可以存取 data、computed
- `mounted` Vue 元件被建立，且渲染在 DOM 上了，可以存取 data、computed、\$ref、\$el
- `destroyed` Vue 元件被銷毀時，如果有 setTimeout、setInterval 時需要在這時 clearTimeout、clearInterval
...
### 內外同步
Vue 2 寫法，元件可以使用 `.sync` 同步外層和內層資料    
```html
...
Vue 3 寫法，在 prop 前使用 `v-model`  
```html
<my-component v-model:mydata="mydata"></my-component>
```
```js
props: {
  data: String
...
Vue.createApp({
  ...
  // context 就是元件本身
  setup(props, context) {
    // ref() 代表 data，可接受所有種類的資料型態，但是不會偵測陣列和物件內部的變化
    // 需要取值時寫成 變數名.value
    const firstName = ref('小明')
    const lastName = ref('王')
    // reactive() 也是 data，只能接受陣列和物件，會偵測陣列和物件內部變化
    // 使用時不需要 .value
