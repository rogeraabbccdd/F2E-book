...
- 安裝 [Heroku CLI](https://devcenter.heroku.com/articles/heroku-cli)，開啟終端機輸入 `heroku login` 後按任意鍵登入
- 在 `Settings` 的 `Config Vars` 輸入機器人 TOKEN 等環境變數
- 在 `Deploy` 綁定 Github 專案
- 若無法綁定 Github
  - 在 `Settings` 的 `App Information` 找 `Heroku git URL`，新增到 Git 遠端儲存庫，命名為 heroku
  - 使用 `git push heroku -f` 上傳
  - **加入 heroku 儲存庫後會被 GitKraken 判斷為私人儲存庫，除非付費否則無法再用 GitKraken 開啟**
- 若部署後看到 `Application Error`，終端機輸入 `heroku logs -a <你的機器人 app 名稱> --tail`
