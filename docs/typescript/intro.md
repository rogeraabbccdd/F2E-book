...
- `target`: 產生出的 JS 語法版本，與模組系統無關
  - 決定是否保留箭頭函數等 JS 新語法
  - 常用 `esnext`
- `moduleResolution`: 如何解析 import 路徑
  - `classic`: 廢棄
  - `node` 或 `node10`: 與 Node.js 原生 require 相同，可不寫副檔名
  - `node16` 以後含 `nodenext`: 與 Node.js 原生 module 相同，不管 package.json 設定的 type 是什麼都必須副檔名
  - `bundler`: 給 `vite` 等打包工具使用的模式，TypeScript 不會解析 module，假設打包工具會處理
...
## 編譯部署
使用 `tsc` 指令，將 TypeScript 編譯成 JavaScript後部署執行  
```json
{
  "scripts": {
    // 先編譯成 JavaScript
